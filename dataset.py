...
def chronological_cv(
    ...
    for i in range(1, num_cv + 1):
        ...
        # Remove classes from the test set for whom the train data is not available.
        updated_test_data, updated_test_owner = filter_with_labels(
            final_test_data, final_test_owner, updated_train_owner
        )

        # Create the data matrix and labels required for the deep learning model training
        ...
        <!--background-image:url({{url_for('static',filename="finnn.jpg")}});
